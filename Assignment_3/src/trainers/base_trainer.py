...
class BaseTrainer:
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            log = {'epoch': self.current_epoch}
            log.update(train_result)
            train_accuracies.append(train_result['accuracy'])  # Assuming accuracy is in train_result

            if self.should_evaluate():
                eval_result = self.evaluate()
                # save eval information to the log dict as well
                log.update({f'eval_{key}': value for key, value in eval_result.items()})    
                val_accuracies.append(eval_result['accuracy'])  # Assuming accuracy is in eval_result
